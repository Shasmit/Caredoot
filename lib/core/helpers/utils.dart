...
class Utils {
  ...
  static Future<bool> isUserLoggedIn() async {
    var username = await UserHelpers.getUserDetails();
    if (username != false) {
      return true;
    }
    return false;
  ...
  static List<TimeSlot> getTimeSlots() {
    ...
    int startingHour = 9;
    int lastHour = 20;
    for (int i = (startingHour); i <= lastHour; i++) {
      if (i > 12) {
        slots.add(TimeSlot(
          time: "${i - 12}:00",
        ));
  ...
  // static DateTime getDateFromHHMMString(String time) {
  //   DateTime currentDate = DateTime.now();
  //   DateFormat format = DateFormat("HH:mm");
  //   DateTime resultDateTime = format.parse(time);
